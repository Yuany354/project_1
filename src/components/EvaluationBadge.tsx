    ...
    items: [
      {
        label: '加强' as const,
        trigger: '【存量同向】+【流量三方加仓】',
        explanation: '主力底仓方向相同，且今日外资/成交量前五/自定义席位同向加仓，合力最显著加强（最关注的）。',
    ...
    items: [
      {
        label: '分化' as const,
        trigger: '【存量同向】+【流量参差不一】',
        explanation: '主力底仓方向一致，但今日三类席位流量变动不一致，出现分化迹象。',
        badgeStyle: 'bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100/50'
      },
      {
        label: '削弱' as const,
        trigger: '【存量同向】+【流量全线减仓】',
        explanation: '主力底仓方向相同，但今日三类席位同步减仓，或日内出现同步大幅反向操作。',
    ...
    items: [
      {
        label: '分歧' as const,
        trigger: '【存量相反】+【三方多空分歧】',
        explanation: '外资、成交量前五会员、用户自定义底仓方向相反，未形成一致持仓结构。',
...
export function getCommodityEvaluation(c: Commodity): EvaluationResult {
  ...
  const isVerySmallChange = Math.abs(fChg) <= 0.1 && Math.abs(iChg) <= 0.1 && Math.abs(rChg) <= 0.1;
  ...
  // 存量方向同向判断 (偏外资、成交量前五、用户自定义三类席位)
  const fDir = fStock > 0 ? 1 : (fStock < 0 ? -1 : 0);
  const iDir = iStock > 0 ? 1 : (iStock < 0 ? -1 : 0);
  const rDir = rStock > 0 ? 1 : (rStock < 0 ? -1 : 0);
  
  const stockSameDirection = fDir !== 0 && fDir === iDir && iDir === rDir;
  const mainDir = fDir;
  ...
  if (!stockSameDirection) {
    ...
    return {
      ...
      itemIndex: 0,
      trigger: '【存量相反】+【三方多空分歧】',
      explanation: '外资、成交量前五、用户自定义席位底仓方向相反，未形成一致持仓结构。',
  ...
  const rIsAdding = rChg * mainDir > 0;

  const fIsReducing = fChg * mainDir < 0;
  const iIsReducing = iChg * mainDir < 0;
  const rIsReducing = rChg * mainDir < 0;

  if (fIsAdding && iIsAdding && rIsAdding) {
    // 加强: 同向加仓
    return {
      label: '加强',
      tierIndex: 0,
  ...
  if (fIsReducing && iIsReducing && rIsReducing) {
    // 削弱: 同向减仓（或者背离反做）
    return {
      label: '削弱',
      tierIndex: 1,
