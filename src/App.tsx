                          ...
                          >
                            ...
                            >
                              <div className="flex flex-col items-center justify-center">
                                {/* Coordinate Number Axis with Foreign and Institutional markers */}
                                <div className="w-full max-w-[125px] h-6 relative flex items-center justify-center select-none">
                                  ...
                                  {/* Institutional dot */}
                                  <div 
                                    className="absolute w-2.5 h-2.5 rounded-full bg-rose-500 border border-white shadow-xs -translate-x-1/2 z-20 hover:scale-130 transition-transform"
                                    style={{ left: `${Math.max(3, Math.min(97, 50 + (iVal / maxPositionLimit) * 50))}%` }}
                                    title={`偏机构持仓: ${iVal >= 0 ? '+' : ''}${iVal.toFixed(1)} 亿`}
                                  ></div>
                                </div>

                                <div className="text-[9px] font-sans text-slate-400 mt-1 flex gap-2 justify-center leading-none">
                              ...
                              {/* STUNNING VISUAL FLOATING TOOLTIP on Hover (Satisfying request detail: hover on axis shows foreign, institutional exact positions) */}
                              <AnimatePresence>
                                {hoveredRowId === c.id && (
                                  <motion.div
                                  ...
                                  >
                                    <div className="font-bold border-b border-slate-800 pb-1 mb-1.5 text-amber-400 flex justify-between items-center">
                                      <span>{c.name} 头寸拆解</span>
                                      <span className="text-[9px] bg-slate-800 text-slate-300 px-1.5 py-0.2 rounded font-normal">DMF</span>
                                    </div>
                                    <div className="space-y-1">
                                      ...
                                      <div className="flex justify-between">
                                        <span className="text-slate-400">偏机构持仓:</span>
                                      ...
                                      <div className="flex justify-between border-t border-slate-800 mt-1 pt-1 text-[10px]">
                                        <span className="text-slate-400">双多空共识:</span>
                                        <span className="text-slate-200">
                                          {hasSameDirection ? (consensusSum >= 0 ? '共同偏多 (买入)' : '共同偏空 (卖出)') : '多空意见分歧'}
                                        </span>
                                      </div>
                                    </div>
                                    {/* Arrow */}
                            ...
                            <td className="p-3 text-[10px]">
                              <div className="flex flex-col gap-1 w-full max-w-[155px]">
                                ...
                                <div className="flex items-center gap-1 leading-none justify-between">
                                  <div className="flex items-center gap-0.5 w-[54px] shrink-0">
                                    <span className="text-[9px] text-slate-400 font-sans">机</span>
                    ...
                    <table className="w-full text-left border-collapse text-xs table-fixed min-w-[1000px]">
                      <thead>
                        <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 font-medium font-sans text-[11px]">
                          ...
                          <th className="p-3 w-36">偏外资持仓头寸</th>
                          <th className="p-3 w-36">偏机构持仓头寸</th>
                          <th className="p-3 w-36">偏散户持仓头寸</th>
                        ...
                        {(compareMode === 'multi' ? selectedCommoditiesList : filteredCommodities).map((c, idx) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="p-3 font-mono">
                                <div className="flex items-center gap-1">
                                  <span className={`text-[9px] px-1 rounded-xs scale-90 ${getBehavior(fVal, fChg).bg}`} title={`外资: ${getBehavior(fVal, fChg).label}`}>
                                    外:{getBehavior(fVal, fChg).label}
                                  </span>
                                  <span className={`text-[9px] px-1 rounded-xs scale-90 ${getBehavior(iVal, iChg).bg}`} title={`机构: ${getBehavior(iVal, iChg).label}`}>
                                    机:{getBehavior(iVal, iChg).label}
                                  </span>
                                  <span className={`text-[9px] px-1 rounded-xs scale-90 ${getBehavior(rVal, rChg).bg}`} title={`散户: ${getBehavior(rVal, rChg).label}`}>
                                    散:{getBehavior(rVal, rChg).label}
                                  </span>
                                </div>
                              </td>
